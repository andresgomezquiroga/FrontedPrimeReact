...
const Sidebar = () => {
    ...
    let items = [
        {
            template: () => {
                return (
                    <span className="inline-flex align-items-center gap-1 px-10 py-2">
        ...
        },
        {
            ...
            template: (item, options) => {
                return (
                    <button onClick={(e) => options.onClick(e)} className={classNames(options.className, 'w-full h-full p-link flex align-items-center p-2 pl-4 text-color hover:surface-200 border-noround')}>
    ...
    return (
        <div className="flex flex-column">
            {visible ? <TieredMenu model={items} className="w-1/8 h-full" /> : null}

            <div className='relative top-2 left-1'>
                <Button icon="pi pi-align-justify" rounded className="p-button-outlined" onClick={openMenu} />
            </div>
        </div>
    )
}
